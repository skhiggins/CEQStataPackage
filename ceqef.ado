...
**          Including sorting at the beginning of ceqbensp sub-program and fixed issue with local
**        Fixed typos in calculations and formulas of IE indicator for poverty
**          Fixed typos in calculations of IE indicator for inequality
**          Fixed algorithm for IE indicator for inequality for combined system
**        Fixed calculation in main program of SE indicator for inequality when there is only taxes
**        Added missing if that was altering the number of matched braces
**        Fixed issue with id_taxharm variable in ceqtaxharm sub-program
**          Fixed issue with scalar in ceqbensp sub-program
**        Eliminated conditions on ginis to calculate Spending Effectiveness indicators
**          Fixed typos in calculation of Spending Effectiveness indicators for inequality
**          Added an if clause so that the program does not calcute Beckerman Imervoll if ppp option is not specified
**          Updated PPP conversion factors to 2011-PPP era
**          Added consideration of special case for optimal tax in ceqtaxstar sub-program
**          Eliminated calculations:
**                1. Net Income to Gross Income
**                2. Any Income concept to Taxable Income
**                3. Taxable Income to any income concept
...
#delimit;
    syntax [if] [in] [pw aw iw fw/] [,
        ...
        if (`tot' > r(sum) | `tot' == 0 ) {
            ...
            local tot2=abs(___cum_diff_y_i[_N])
            
    
                        
        if (`tot'>`tot2') {  // Considering special case - June 24,2020
            
            return scalar t_N=1
            
            gen ___taxed = (abs(___cum_diff_y_i) < abs(___cum_diff_y_i[_N]))
            ...
            local still_tax =  abs(___cum_diff_y_i[_N]) - abs(___cum_diff_y_i[`which'])
            ...
            local dif=`tot'- abs(___cum_diff_y_i[_N])
            
            local dif_per_person=`dif'/___cum_w[_N]
            replace ____taxstar = ____taxstar +  `dif_per_person'
            replace ___taxed=1
        ...
        // Determining who is taxed.
            ...
            // Ensuring that we are allocating the exact amount of tax available
    ...
    syntax [if] [in] [pw aw iw fw/] [,
        ...
        *no taxes income
        ...
        if `tot' > r(sum) { ;
            ...
            replace ____id_taxharm=1 in `=`which' + 1' // It was not considering the last one taxed - April 7th, 2020
    ...
    syntax [if] [in] [pw aw iw fw/] [,
        ...
        *See if we are dealing with taxes or transfers
        ...
        if `id_ben' == 1 {
            ...
            replace `S'=0 if _n==_N // Taking care of the last observation - May 7 , 2020
            qui gen double `T' = `S'*`D' in 1
            qui replace `T' = `S'*`D' + `T'[_n-1] if _n>1 // Sum of differnce in OI.

            tempvar F_hat w_F
            qui gen double `F_hat' = `D'[_n-1] + `C'/2 // empirical distribution / rankings; H
            qui replace `F_hat' = `C'/2 in 1 // H
            qui gen double `w_F' = `C'*`F_hat' // weighted rankings
            qui summ  `w_F'
            scalar Fbar = r(sum) // average weighted rankings; J and K
            ...
            qui covconc `E' [pw=`C']
            scalar Gini_OI = r(gini) // O

            qui summarize `E' [aw=`C'], meanonly
            scalar mu_OI = r(mean) // M

            tempvar Gini_star

            qui gen double `Gini_star' = (mu_OI*Gini_OI + `AD')/(mu_OI + `T') //AF
            replace `Gini_star' =0 if (mu_OI + `T')==0 // Adjusting to Ali's excel file - May 6, 2020
            

            // suppose the observed ending income Gini is saved in scalar Gini_EI
            qui covconc `G' [pw=`C']
            scalar Gini_EI = r(gini) // P

            tempvar higher_gini
            qui gen byte `higher_gini' = (`Gini_star' < Gini_EI) & /// AG
                                   (`Gini_star'[_n-1] > Gini_EI )
            replace `higher_gini'=0 if _n==1 // Adjusting to Ali's excel file - May 7, 2020
            ...
            local which = r(mean) */ // Adjusting to Ali's excel file - May 7, 2020

                        
            // Composition of s
            // signs change depending on tax or benefit
            
            // Adjusting to Ali's excel file - May 7, 2020
            
            tempvar AO AQ AS AU AW
            
            qui gen double `AO'= `B'*`higher_gini'
            qui summ `AO'
            
            if r(sum)>0 scalar AP2=r(sum)
            else scalar AP2=1
            
            local which=AP2
            
            qui gen double `AQ'= `D'*`higher_gini'
            qui summ `AQ'
            scalar AR2= r(sum)
            
            qui gen double `AS'= (mu_OI*Gini_OI+ `AD')*`higher_gini'[_n+1]
            qui summ `AS'
            scalar AT2= r(sum)
            
            qui gen double `AU'=(mu_OI + `T')*`higher_gini'[_n+1]
            qui summ `AU'
            scalar AV2= r(sum)
            
            qui gen double `AW'=2*`X'*`higher_gini'
            qui summ `AW'
            scalar AX2 = r(sum)
            ...
            qui gen double `AH'= `E'* `higher_gini'
            qui summ `AH'
            scalar AI2=r(sum)
            
            qui gen double `AJ' = (AI2 - `E')*(`E' <= AI2)

            qui gen double `AK' = `E' + `AJ'
                        
            
            scalar Q=Gini_OI-Gini_EI

            
            tempvar AZ BA
                    
            local scondition= Gini_EI*AR2-AX2
            
            if `scondition'!=0 local s = (AT2 - Gini_EI*AV2)/(Gini_EI*AR2 - AX2) // AY; Adjutsitng to new formula - May 7, 2020
            else local s=(mu_OI*Q)/(`C'* (Gini_OI-Q)-2*`W') in `=`which'' //AY
        ...
        if `id_tax' == 1 {
            *set trace on
            ...
            replace `F_hat' = `C'/2 in 1 // I and J
            tempvar w_F
            gen double `w_F' = `C' * `F_hat'
            summ `w_F'
            scalar Fbar = r(sum) //K and L
            assert abs(r(sum)-.5)/.5 < 0.00001
            
            
            summarize `E' [aw=`C'], meanonly
            scalar mu_OI = r(mean) // N
            
            
            // Storing value for starting income Gini
            covconc `E' [pw=`C']
            scalar Gini_OI = r(gini)  //P
            
            // suppose the observed ending income Gini is saved in scalar Gini_EI
            covconc `G' [pw=`C']
            scalar Gini_EI = r(gini) //Q
        
            
            tempvar  S
            gen double `S' =  `E' - `E'[_n-1] // T

            gsort - `E'
            tempvar Drev T
            gen double `Drev' = sum(`C') // E
            gen double `T' = `S'*`Drev' in 1
            replace `T' = `S'*`Drev' + `T'[_n-1] if _n>1 // U
            
            
            tempvar W
            gen double `W' = (`F_hat' - Fbar)*`C' // X
            
            tempvar X
            gen double `X' = `W' in 1
            replace `X' = `W' + `X'[_n-1] if _n>1 // Y

            tempvar Y Z
            gen double `Y' = `X'*`S' // Z
            gen double `Z' = `Y' in 1
            replace `Z' = `Y' + `Z'[_n-1] if _n>1 // AA

            tempvar AD
            gen double `AD' = 2*`Z' // AE
            ...
            tempvar Gini_star
            gen double `Gini_star' =0
            replace `Gini_star'= (mu_OI*Gini_OI - `AD')/(mu_OI - `T') if (mu_OI - `T')!=0 // AG
            ...
            tempvar higher_gini
               gen byte `higher_gini' = (`Gini_star' < Gini_EI) & (`Gini_star'[_n+1] > Gini_EI) // AH
               replace `higher_gini' = (`Gini_star' < Gini_EI) if _n==_N // AH; Adjutsitng to new formula - May 6, 2020
                *qui gen byte `higher_gini' = (`Gini_star' < Gini_EI) & (`Gini_star'[_n-1] > Gini_EI )

            summ `B' if `higher_gini'==1
            assert r(min)==r(max) // just one obs
            local which = r(mean)

            tempvar AP AT AV // Adjusting to Ali's excel file - May , 2020
            gen double `AP'=(`B'[_N]+1-`B')*`higher_gini'
            qui summ `AP'
            scalar AQ2= r(sum)
            gen double `AT'=(Gini_OI*mu_OI- `AD')* `higher_gini'[_n-1]
            qui summ `AT'
            scalar AT2= r(sum) //AU
            gen double `AV'=(mu_OI - `T')* `higher_gini'[_n-1]
            qui summ `AV'
            scalar AV2= r(sum) //AW
            
            
            scalar AR2 = `Drev' in `which' // AS
            scalar AX2 = 2*`X' in `which' // AY

                        

            if `=AQ2'>1 local s = (AT2 - Gini_EI*AV2)/(AX2-Gini_EI*AR2)    //AZ;  Adjuting to Ali's excel file - May 6, 2020
            else local s=(mu_OI*(Gini_OI-Gini_EI))/(2*AR2*(`F_hat'[_N] - Fbar)- AR2*Gini_EI)

            tempvar AJ AK AZ BA
            gen double `AJ' = (`E'[`which'] - `E')*(`B' >=`which') // AK

            gen double `AK' = `E' + `AJ' // AL

            gen double `AZ' = (`AJ' - `s')*(`B'>=`which') // BA

            gen double `BA' = `AZ' + `E' // BB
            ...
            return scalar se_ind = tot_EHB / tot_TB // return was missing- Feb 15,2020
...
program define ceqbensp, rclass
    ...
    #delimit cr
        ...
        sort `startinc' // adding sorting by original income_ Apr 20th,2020.
        ...
        if     "`obj1'" != "" {
            ...
            tempvar opt_ben
            gen double `opt_ben' = min(`H',A-`G')*(`G'<A) // AW; optimal benefits according to alis agorithm in Spending Effectiveness for the poverty indicators_Nov 24, 2017.
            ...
            return scalar sp_ef_pov_1 = prime1/tot_ben1 //AX
        ...
        if "`obj2'" != "" {
            ...
            scalar O2 = `r(mean)' // For some reason, it was running into problems if the naem was O- April 20th, 2020

            scalar P = N-O2 // Marg contribution between OI and EI
            local P = N-O2  // Also using local because there have been issues with using scalar.
            ...
            tempvar Z AB    // Adjusting it to Ali's excel file - May 10, 2020
            
            qui gen double `Z'=`Y'*(`C'+1)
            qui summ `Z'
            if (r(sum)==0) scalar AA=1
            else scalar AA=r(sum)

            qui gen double `AB'= `X'*`Y'
            qui summ `AB'
            scalar AC= P- r(sum)
            ...
            tempvar AE AF AI AN
            qui gen `AE' = (`X' > P) in 1 // Adjusted Y column
            qui replace `AE' = (`X' > P & `X'[_n-1] < P)  if _n>1 // Adjusted Y column
            
            qui gen double `AF'=`S'*`AE'
            qui summ `AF'
            scalar AG= r(sum)
            scalar AH = AG^2 // (z-y_J)^2
            
            
            qui gen double `AI'=`F'*`AE'
            qui summ `AI'
            scalar AJ=r(sum)
            ...
            if min(AK,AL)>0 scalar AM=min(AK,AL) //Changing to match excel
            else scalar AM= max(AK,AL)
    
            qui gen double `AN'= `G'*`AE'
            qui summ `AN'
            scalar AO= r(sum)
            ...
            scalar drop E N O2 P Q AA AC AD AG AH AJ AK AL AM AO AT prime2 tot_ben2
...
* general programming locals;
    ...
    **********
/*    From July 22nd on, the following taxes and transfers will not be calculated:
    ...
    3 and 4 imply that taxdif will no longer be defined since it's not an actual
    fiscal intervention.
...
*/
                        ...
                        if wordcount("`tax_`rw'_`cc''")>0 & wordcount("`ben_`rw'_`cc''")==0{;
                            ...
                            *If Marg. Cont. is negative, SE is missing;
                            ...
                            if r(mc_ineq)<0{;
                                ...
                                cap drop ___t ;
                                gen double ___t = ``rw'' - abs(`tax_`rw'_`cc'') ; // It was adding taxes- April 1st, 2020
                                covconc ___t `pw' ;
                                local gini1 = r(gini) ;
                                covconc ``rw'' `pw' ;
                                local gini2 = r(gini) ;
                                _ceqspend `pw',inc(``cc'') sptax(`tax_`rw'_`cc'');
                                local seind=r(se_ind);  // SE indicator is different when it's only taxes (se_ind) rather than only transfers (sp_ef)- Feb 15, 2020
                                matrix `rw'_ef[17,`_`cc''] =`seind';
                        ...
                        if wordcount("`tax_`rw'_`cc''")==0 & wordcount("`ben_`rw'_`cc''")>0{;
                            ...
                            *If Marg. Cont. is negative, SE is missing;
                            ...
                            if r(mc_ineq)<0{;
                                ...
                                cap drop ___t ;
                                gen double ___t = ``rw'' + abs(`ben_`rw'_`cc'') ; // It was substracting benefits- April 1st, 2020
                                covconc ___t `pw' ;
                                local gini1 = r(gini) ;
                                covconc ``rw'' `pw' ;
                                local gini2 = r(gini) ;
    ...
    ****************;
    if `"`using'"'!="" {; // "
        ...
        // Export to Excel Poverty lines - October 15, 2020
        local plcol = 2;  // Column number where poverty lines are printed
        returncol `plcol';
        local _plcol `r(col)';
            
        
        local rpovlines_ie =13; // IE poverty lines
        local rpovlines_se=29; // SE poverty lines
        local rpovlines_beck=43; // Beckerman Immervoll poverty lines
        local rpovlines_FI=72; // FI poverty lines
        
        local totalrows=84; // Number of rows until the next set of poverty lines
        
        local rpovlines rpovlines_ie rpovlines_se rpovlines_beck rpovlines_FI;
        local counter=1;
        
        foreach rpl of local rpovlines {;
        
            if `counter'==1 local rows=2; // rows between each poverty line. It is different for each each indicator
            if `counter'==2 local rows=2;
            if `counter'==3 local rows=4;
            if `counter'==4 local rows=3;
            ...
            forval j=1/8{;
                forval i=1/3 {;
                    ...
                    if `i'==1 & `j'==1 {;
                        local all_povlines `all_povlines' `_plcol'``rpl''=("$`pl`i'' PPP");
                        local rpl = ``rpl'' + `rows';
                    };
                    else{;
                        local all_povlines `all_povlines' `_plcol'`rpl'=("$`pl`i'' PPP");
                ...
                local rpl = `original' + `totalrows'*`j';
                local lpovlines `lpovlines' `all_povlines';
      ...
      qui putexcel `lpovlines'  using `"`using'"', keepcellformat modify sheet("`sheet'"); // printing poverty lines
