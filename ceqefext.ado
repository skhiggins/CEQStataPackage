...
*! v1.13 21jun2020 For use with Aug 2017 version of Output Tables
...
* v1.13 Solved problem with ceqtaxharm sub-program for properly identifying taxed people
*        Added missing return to _ceqspend sub-pogram
*        Solved problem with ceqbensp sub-program
*        Fixed bug when calculating IE indicators for inequality
*        Fixed bug when calculating IE indicators for poverty when there is only taxes
*        Adapted _ceqspend to new Ali's excel files
*        Fixed error when calculting SE indicator for inequality
*        Adapted ceqbensp to Ali's excel file and solved problem
* NOTES
*
...
replace ____id_taxharm=1 in `=`which' + 1' // It was not considering the last one taxed - April 25th, 2020
...
tempvar Z AB    // Adjusting it to Ali's excel file - June 20, 2020
...
covconc __y1__ `pw';//gini of column income; income concept was wrong - April 24th, 2020
local g1_`y'=r(gini);
covconc __yo__ `pw';//gini of row income;
local g2_`y'=r(gini); // corrected it local - April 24th, 2020
covconc ___ystar `pw';//gini of star income;
local g_star=r(gini);
local imef=(`g2_`y''-`g1_`y'')/(`g2_`y''-`g_star'); // corrected it local - April 24th, 2020
matrix g_ie_`y'[1,`col'] =`imef';
} ;



*local twarn = r(twarn) ;  // Unnecessary and generates error - May 9, 2020
...
covconc __y1__ `pw' ;
local gini2 = r(gini) ;
// condition errased and _ceqspend modified - May 7, 2020
_ceqspend `pw',inc(__y1__) sptax(__int_tax__) ; // Marc: remove capture once debugged fully. ;
*set trace off ;
*local spef=r(sp_ef);
matrix g_se_`y'[1,`col'] = r(se_ind); // Output for taxes is different - May 8, 2020
...
if !(r(th_0)==1 | r(th_gr)==1) {;  // Missing condition- May 6, 2020
...
local mst_p`i'_h=`p`i'_`y'_2' - `p`i'_h';//Ideal MC with tax formula; The formula was wrong- April 25th, 2020
...
local g1_`y'=r(gini);
covconc __yo__ `pw';//gini of row income;
local g2_`y'=r(gini); // corrected local - April 24th, 2020
covconc ___ystar `pw';//gini of star income;
local g_star=r(gini);
local imef=(`g2_`y''-`g1_`y'')/(`g2_`y''-`g_star'); // corrected local - April 24th, 2020
matrix g_ie_`y'[1,`col'] =`imef';
} ;
...
// Condition errased and _ceqspend modified- Ma 7, 2020
