...
{synopt :{opth pl3(real)}}The highest of three poverty lines to be graphed (and the maximum income included in the graph) expressed in PPP dollars per day (default is $5.50 PPP per day).{p_end}
...
{opth pl3(real)} for the highest of the three poverty lines (the default is $5.50 PPP per day);
the highest poverty line also determines the highest income included in the graphs.
For example, to change the lowest poverty line from $1.90 to $1.25, specify {cmd:pl1(1.25)}.
