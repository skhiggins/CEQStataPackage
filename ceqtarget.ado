...
*! v2.3 03aug2020 For use with Jul 2017 version of CEQ Master Workbook 2017
...
** CHANGES
**     03-08-2020 Re-wrote calcultaions of total benefits in LCU and PPP
**     09-06-2020 Added warning on hh level markers
**     23-04-2020 Fixed issue with totals in targets
...
local v_fiscal_names `v_fiscal_names' v_`cat'
...
local empty=""  // just keeping the categories with variables - August 3rd,2020
foreach var of local `bc'{

if wordcount("``var''")!=0 local empty=`empty'+1
...
local v_broadcat_names `v_broadcat_names' v_`bc'
...
foreach var in `recvars1' `payvars1' {
tempvar tarb_`var'
qui by `hhid' : egen `tarb_`var'' = total(`tarbtemp_`var'') // counts number of targets per hh
local tarb_tokeep `tarb_tokeep' `tarb_`var''
}
foreach var in `recvars2' `payvars2' {
tempvar db_`var'
qui by `hhid' : egen `db_`var'' = total(`dbc_`var'') // counts number of direct ben per hh
local db_tokeep `db_tokeep' `db_`var''
}
foreach var in `recvars2' `payvars2' {
tempvar tar_`var'
qui by `hhid' : egen `tar_`var'' = total(`tarc_`var'') // counts number of targets per hh
local tar_tokeep `tar_tokeep' `tar_`var''
}
foreach var in `recvars2' `payvars2' {
tempvar bdt_`var'
qui by `hhid' : egen `bdt_`var'' = total(`bdtc_`var'') // counts number of direct ben per hh
local bdt_tokeep `bdt_tokeep' `bdt_`var''
}
// Broad Categories
foreach var of local tarb_vlist {
tempvar temp`var'
qui by `hhid': egen `temp`var'' = total(`var') // counts number of members who are target for at least one program in bc per hh
qui replace `var' = `temp`var''
local tarb_tokeep `tarb_tokeep' `var'
}
foreach var of local db_vlist {
tempvar temp`var'
qui by `hhid': egen `temp`var'' = total(`var') // counts number of members who are direct beneficiaries for at least one program in bc per hh
qui replace `var' = `temp`var''
local db_tokeep `db_tokeep' `var'
}
foreach var of local tar_vlist {
...
local counter_fiscal= 1 // New counters - August 3, 2020
local counter_bc= 1 // New counters - August 3, 2020
local counter_fiscal_ppp= 1 // New counters - August 3, 2020
local counter_bc_ppp= 1 // New counters - August 3, 2020
...
// BENEFITS MATRICES - Re-wrote calculations - August 3, 2020 - Caterina Brest Lopez
...
if strpos("`v_fiscal'","`pr'")!=0 {   // Fixing issue with totals - August 3, 2020

local ww: word `counter_fiscal' of `v_fiscal_names'

local yy=substr("`ww'",3,length("`ww'"))
g double `yy'_`v'_t=0

foreach var of local `yy' {

if "`tarb_`var''"!="" {
cap confirm variable `tarb_`var''
if _rc == 0 {
replace `yy'_`v'_t=`yy'_`v'_t+`var'*(`tarb_`var''>0)
}

else{
replace `yy'_`v'_t=`yy'_`v'_t
}
}

}
local ++counter_fiscal
}
if strpos("`v_broadcat'","`pr'")!=0 { // Fixing issue with broad catergories' totals - August 3, 2020

local uu: word `counter_bc' of `v_broadcat_names'
local ee=substr("`uu'",3,length("`uu'"))

g double `ee'_`v'_t=0

foreach var of local `ee' {

foreach var2 of local `var'{

if "`tarb_`var2''"!="" {
cap confirm variable `tarb_`var2''
if _rc == 0 {
replace `ee'_`v'_t=`ee'_`v'_t+`var2'*(`tarb_`var2''>0)
}

else{
replace `ee'_`v'_t=`ee'_`v'_t
}
...
forval gp=1/6 { //By income groups - August 3, 2020

if strpos("`v_fiscal'","`pr'")!=0 {

qui summ `yy'_`v'_t if ``v'_group2'==`gp' & `tarb_`pr'' > 0  `aw'
...
else if  strpos("`v_broadcat'","`pr'")!=0 {

qui summ `ee'_`v'_t if ``v'_group2'==`gp' & `tarb_`pr'' > 0  `aw'
matrix `mat'`v'[`pr_row',`gp'] = r(sum)
}
...
// Totals - August 3, 2020
if strpos("`v_fiscal'","`pr'")!=0 {
qui summ `yy'_`v'_t if `tarb_`pr'' > 0  `aw'
matrix `mat'`v'[`pr_row',7] = r(sum) // total
}

else if  strpos("`v_broadcat'","`pr'")!=0 {
qui summ `ee'_`v'_t if `tarb_`pr'' > 0  `aw'
matrix `mat'`v'[`pr_row',7] = r(sum) // total
}
...
local ww: word `counter_fiscal_ppp' of `v_fiscal_names'

local yy=substr("`ww'",3,length("`ww'"))
g double `yy'_`v'_ppp_t=0
...
cap confirm variable `tarb_`var''
if _rc == 0 {
replace `yy'_`v'_ppp_t=`yy'_`v'_ppp_t+``var'_ppp'*(`tarb_`var''>0)
}

else{
replace `yy'_`v'_ppp_t=`yy'_`v'_ppp_t
}
}

}
local ++counter_fiscal_ppp
}
if strpos("`v_broadcat'","`pr'")!=0 { // Fixing issue with broad catergories' totals - August 3, 2020

local uu: word `counter_bc_ppp' of `v_broadcat_names'
local ee=substr("`uu'",3,length("`uu'"))

g double `ee'_`v'_ppp_t=0
...
cap confirm variable `tarb_`var2''
if _rc == 0 {
replace `ee'_`v'_ppp_t=`ee'_`v'_ppp_t+``var2'_ppp'*(`tarb_`var2''>0)
}

else{
replace `ee'_`v'_ppp_t=`ee'_`v'_ppp_t
}
...
qui summ `yy'_`v'_ppp_t if ``v'_group2'==`gp' & `tarb_`pr'' > 0  `aw'
...
qui summ `ee'_`v'_ppp_t if ``v'_group2'==`gp' & `tarb_`pr'' > 0  `aw'
matrix `mat'`v'_ppp[`pr_row',`gp'] = r(sum)
}
...
// Totals - August 3, 2020
if strpos("`v_fiscal'","`pr'")!=0 {
qui summ `yy'_`v'_ppp_t if `tarb_`pr'' > 0  `aw'
matrix `mat'`v'_ppp[`pr_row',7] = r(sum) // total
}

else if  strpos("`v_broadcat'","`pr'")!=0 {
qui summ `ee'_`v'_ppp_t if `tarb_`pr'' > 0  `aw'
matrix `mat'`v'_ppp[`pr_row',7] = r(sum) // total
}

else {
qui summ ``pr'_ppp' if `tarb_`pr'' > 0 `aw'
matrix `mat'`v'_ppp[`pr_row',7] = r(sum) // total
}
} //if _rc == 0
} // if "`tarb_`pr''"!=""
